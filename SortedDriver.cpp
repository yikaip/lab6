...
mostIsolated(vector<double> & number)
{
	...
	double IsoDiff = abs((number.at(number.size() - 2)) - (number.at(number.size() - 1))); //to return the difference between the next number and number
	...
	for (int i = (number.size()-1); i >= 0; i--)  //for integer location i starting at size-2, bigger or equals to 0, derease 1 each time
	{
